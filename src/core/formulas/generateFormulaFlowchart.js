...
const {htmlFormulaTemplate} = require('../../utils/mermaidTemplate');
const align = require('../../utils/stringAlign');
...
const sortSteps = (trigger, steps) => {
  try {
    ...
    while (length(sortedSteps) !== cnt) {
      ...
      const step = getStep(sortedSteps[cnt]);
    ...
    logError('Error occured at formula sorting step');
    throw error;
  }
};
...
module.exports = async (formula, formulaDirName) => {
  try {
    ...
    const parsedSteps = concat([parseTrigger(formula.triggers[0])], chain(parseStep, sortedSteps));
    ...
    logError('Failed to generate formula flow chart');
    throw error;
  }
};
