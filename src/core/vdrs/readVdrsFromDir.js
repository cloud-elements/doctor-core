...
const buildVdrV2FromDir = async dirName => {
  const vdrs = {};
  const vdrNames = await promisify(readdir)(dirName);
    ...
    if (isDirectory(elementFolder)) {
      ...
      for (const name of transformations) {
        ...
        if (isDirectory(transformationFolder) && existsSync(transformationFile)) {
          ...
          result = assocPath([element, name], transformation, result);
        }
      }
    }
  }
...
module.exports = async (dirName, vdrNames) => {
  if (existsSync(`${dirName}/objectDefinitions`) && existsSync(`${dirName}/transformations`)) {
